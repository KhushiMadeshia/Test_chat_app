...
class MessageActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Handle system window insets to avoid overlap with status bar and navigation bar
        val rootView = findViewById<android.view.View>(R.id.main)
        androidx.core.view.ViewCompat.setOnApplyWindowInsetsListener(rootView) { v, insets ->
            val systemBars = androidx.core.view.WindowInsetsCompat.Type.systemBars()
            val insetsValue = insets.getInsets(systemBars)
            v.setPadding(insetsValue.left, insetsValue.top, insetsValue.right, insetsValue.bottom)
            insets
        }
